...
from config import TOKEN, MAIN_MENU, METRO_STATIONS, MANAGER_ID, catalog,\
                   ADMIN_PANEL, check_time, DEVELOPERID, DEVELOPER_PANEL
...
async def msg_handler_func(message: types.Message):
    ...
    #* manager commands 
    if message.from_id == MANAGER_ID:
        if message.text.lower() == 'перезагрузить каталог🔄' and message.from_id == MANAGER_ID:
            print('btn3')
            try:
                update_database()
